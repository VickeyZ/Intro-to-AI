...
def CNN(input_text,keep_prob):
    ...
    with tf.name_scope("CNN"):
        # CNN layer
        # 首先是一维的卷积操作，卷积核的大小以及滤波器的个数参考配置文件，卷积步长默认为1
        # 利用函数tf.layers.conv1d
        # shape: b x len x emd->b x len1 x chanel
        conv1 = tf.layers.conv1d(embedding_input, config.num_filters, config.kernel_size, 1)
        # 然后是最大池化操作，池化的大小和池化的步长参考配置文件
        # 利用函数tf.layers.max_pooling1d
        # shape: b x len1 x chanel->b x len2 x chanel
        pool1 = tf.layers.max_pooling1d(conv1, config.pool_size, config.pool_stride)
        # 最后利用1x1卷积将所有通道的特征叠加
        # 利用函数tf.layers.conv1d，设置卷积核和滤波器均为1
        # shape：b x len2 x chanel->b x len2 x 1
        conv2 = tf.layers.conv1d(pool1, 1, 1, 1)
        # 利用tf.squeeze函数将矩阵的第三维1裁减掉，起到从三维到二维的降维操作
    ...
    with tf.name_scope("MLP_output"):
        ...
        # 第二层全连接也就是最后输出的分类层
        # 利用函数tf.layers.dense
        output_logits = tf.layers.dense(fc1, units=config.num_classes)
        ...
        # 利用函数tf.argmax
        pred_label = tf.argmax(norm_logits, 1)
...
if __name__=='__main__':
    ...
    # -------------------定义多层感知机的模型---------------
    # 调用CNN()函数，在该函数中完成输入到输出的定义
    output_logits, pred_label = CNN(input_text, keep_prob)
    # ---------------------------------------------------
    ...
    with tf.name_scope("accuracy"):
        ...
        # 正确分类的数据由独热编码转为标签
        # 利用函数tf.argmax
        true_label = tf.argmax(input_label, 1)
